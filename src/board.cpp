...
void Board::makeMove(const Move& m)
{
    ...
    hashKey ^= pieceKeys[m.piece][m.from];
    hashKey ^= pieceKeys[m.piece][m.to];
    ...
    {
        if (!(m.flags & EN_PASSANT)) hashKey ^= pieceKeys[m.capturePiece][m.to];
    ...
    {
        hashKey ^= pieceKeys[m.piece][m.to];
        hashKey ^= pieceKeys[m.promotionPiece][m.to];
